...
const _connect = (UIComponent,...reducerOptions)=>{
    let mapStateToProps = (state)=>{
        //如果我们使用的时候什么都没传，connect（TodoConent）把所有reducer的数据都给他
        ...
        reducerOptions.forEach(reducer => {
            if(  (typeof reducer ) === 'string' ){//如果这个元素是一个字符串    ，那就是对应的reducer的名字
                   if(state[reducer]){ //如果这个reducer对应的状态存在的话
    ...
    let mapDisPatchToProps = (dispatch) => {
        ...
        reducerOptions.forEach(reducer => {
            if(  (typeof reducer ) === 'string' ){//如果这个元素是一个字符串    ，那就是对应的reducer的名字
                   if(actionCreators[reducer ]){ //如果这个reducer对应的状态存在的话
